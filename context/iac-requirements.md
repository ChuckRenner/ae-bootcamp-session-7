...
The todo-service is a stateless full-stack task management application: a React SPA frontend and a Node.js/Express REST API for CRUD operations on todos. The API uses an in-memory store for this lab. It will be containerised and hosted on AWS ECS Fargate behind an Application Load Balancer, with CloudWatch logs and Container Insights providing observability.
