...
terraform {
  ...
  #
  # The key is injected by CI when the stack is initialized.
  #
  backend "s3" {
...
# ---------------------------------------------------------------
# The AWS provider uses short-lived credentials from the environment when
# Terraform runs in GitHub Actions via OIDC.
# ---------------------------------------------------------------
provider "aws" {
